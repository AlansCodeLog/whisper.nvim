...
M.setup = function(user_config)
  local config = require('speak-to.config').setup(user_config)

  -- Set up keybindings
  for _, mode in ipairs(config.modes) do
    vim.keymap.set(mode, config.keybind, function()
      require('speak-to.audio').toggle_recording(config)
    end, { desc = 'Toggle speech-to-text recording' })
  end

  -- Register user commands
  vim.api.nvim_create_user_command('SpeakToToggle', function()
    require('speak-to.audio').toggle_recording(config)
  end, { desc = 'Toggle speech-to-text recording' })

  vim.api.nvim_create_user_command('SpeakToDownloadModel', function(opts)
